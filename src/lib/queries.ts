...
export type SavedItem = { id: string; source: string; url: string | null; author: string | null; kind: string; title: string; topic: string | null; summary: string | null; key_points: string[] | null; tags: string[] | null; image_url: string | null; image_urls: string[] | null; video_url: string | null; note: string | null; favorite: boolean; done: boolean; position: number; created_at: string };

const SAVED_FULL = "id, source, url, author, kind, title, topic, summary, key_points, tags, image_url, image_urls, video_url, note, favorite, done, position, created_at";
...
export async function getEntityMeta(userId: string, kind: "people" | "places"): Promise<Record<string, { id: number; hidden: boolean; lat?: number | null; lng?: number | null }>> {
  ...
  try {
    ...
    if (error) throw error;
    for (const r of data || []) out[r.name] = { id: r.id, hidden: !!r.hidden };
    return out;
  } catch {
    try {
      const { data } = await supabaseAdmin().from(kind).select("id, name").eq("user_id", userId);
      for (const r of data || []) out[r.name] = { id: r.id, hidden: false };
    } catch {}
    return out;
  }
}
