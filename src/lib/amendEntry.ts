...
const MARKERS =
  /(исправь|поправь|поправк|уточняю|уточнение|уточню|уточнить|на самом деле|не наоборот|ошибся|ошибк[аи] в|опечатк|имел[аи]? в виду|перепутал|корректир|это не так|не верно|виправ|уточнюю|насправді|переплута|actually|correction|i meant|to clarify|let me clarify|fix that|that'?s wrong|that is wrong|en fait|je voulais dire|erreur)/i;
