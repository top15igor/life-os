...
// Статическая «шапка» разбора — одинакова для всех пользователей и всех записей.
// Вынесена в system-блок с кэшированием (prompt caching): при повторных вызовах
// эта часть + схема инструмента читаются из кэша (~10% цены), а не оплачиваются заново.
...
- finance: ПРЯМАЯ ПРОСЬБА ЗАПИСАТЬ ТРАТЫ — это тоже реальные операции, и самые бесспорные из всех. «Запиши сегодняшние расходы: бассейн 25 евро, продукты 90 евро, проезд 20 евро», «занеси траты: кофе 4, такси 15» — человек диктует свои расходы, каждый названный предмет с суммой заводи отдельной операцией. Вежливая обёртка («можешь, пожалуйста, записать…») ничего не меняет: это не рассказ и не размышление, а перечень трат. Помимо этого — реальная трата/доход, совершённые сейчас/сегодня и названные как факт: «потратил 500 на продукты», «кофе за 80», «заплатил за аренду 12000» → expense; «получил зарплату 50000», «заработал 3000» → income. Укажи amount, category, currency (если ясна). ВАЖНО: не превращай в операцию суммы из рассказа — итоги за годы («вложили 200 тысяч за 7 лет»), планы («хочу вложить»), стоимость бизнеса, долги, крупные инвестиции-воспоминания. Нет реальной операции прямо сейчас или сомневаешься — НЕ добавляй.
...
export async function analyze(text: string, userId?: string): Promise<Analysis> {
  ...
  const attempt = async (model: string): Promise<Analysis> => {
    const msg = await client().messages.create({
      model,
      max_tokens: 1500,
      // Кэшируем схему инструмента + системную «шапку» (одна кэш-точка на последнем
      // system-блоке покрывает и tools, и system, т.к. tools идут раньше в префиксе).
