...
// Контекст нужен ТОЛЬКО когда человек на что-то ссылается. Иначе он вредит:
// агент-исследователь поймал это на первом же прогоне — фраза «потратил 500,
// а не 800» без объекта подтянула из памяти напоминание «позвонить маме», и
// бот пошёл искать трату про маму. Память диалога должна включаться по слову-
// отсылке, а не висеть фоном на каждом сообщении.
// ВАЖНО: границы слова через \b и класс \w в JavaScript работают только с
// латиницей — на кириллице они молча не срабатывают. Поэтому юникодные
// просмотры по буквам: без них проверка выглядела бы рабочей и не ловила ничего.
const REF_RE = /(?<![\p{L}\p{N}])(?:это|этот|эту|этого|этим|тот|ту|того|его|её|ее|их|там|туда|тут|сюда|первый|первое|первую|второй|второе|вторую|третий|третье|последний|последнее|последнюю|предыдущ[\p{L}]*|оба|обе|все)(?![\p{L}\p{N}])/iu;
