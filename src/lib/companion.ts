...
// Тон общения с ботом, выбранный пользователем в профиле (по умолчанию — «тёплый друг»).
async function getChatTone(userId: string): Promise<MorningTone> {
  try {
    const { data } = await supabaseAdmin().from("users").select("morning_prefs").eq("id", userId).maybeSingle();
    return normalizeMorningPrefs((data as any)?.morning_prefs).chatTone;
  } catch {
    return "friend";
  }
}
...
function personaText(who: string, lang: Lang, tone: MorningTone = "friend"): string {
...
СТИЛЬ РЕЧИ (пользователь выбрал его сам в профиле): ${TONE_PROMPT[tone]}. Держись этого стиля во всех ответах — это главное в том, как ты звучишь.
...
function buildSystem(name: string | null, context: string, off?: number | null, lang: Lang = "ru", tone: MorningTone = "friend"): string {
  const who = name ? name : "собеседника";
  return `${personaText(who, lang, tone)}\n\n${knowBlock(context, off)}`;
}
...
export async function buildVoiceInstructions(
  ...
  const [context, chatTone] = await Promise.all([gatherContext(userId), getChatTone(userId)]);
  const base = buildSystem(name, context, tzOffset, lang, chatTone);
  return `${base}
...
export async function talkToCompanion(
  ...
  const [context, history, recall, chatTone] = await Promise.all([
  ...
  const who = name ? name : "собеседника";
  const system: any = [
    { type: "text", text: personaText(who, lang, chatTone), cache_control: { type: "ephemeral" } },
