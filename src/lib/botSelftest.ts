...
const SCENARIOS: Scenario[] = [
  {
    // «Надо, чтоб Джарвис всегда присутствовал: задал вопрос — он подключается,
    // и звать его отдельно не надо». Проверяем на живом случае: вопрос про мир
    // посреди обычной записи должен получить настоящий ответ.
    name: "Джарвис сам отвечает на вопрос внутри записи",
    heavy: true,
    chain: () => [message("собираем вещи, завтра вылетаем в Исландию к друзьям. А Исландия это остров или материк?")],
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const joined = texts(sent).join("\n").toLowerCase();
      if (!/остров/.test(joined)) return "вопрос из записи остался без ответа — Джарвис не подключился";
      return null;
    },
  },
  {
    // Джарвис должен не только рассуждать, но и делать. «Только рассуждает» —
    // это справочная, а не помощник.
    name: "Джарвис: не только думает, но и делает",
    heavy: true,
    chain: () => [message("джарвис, поставь напоминание завтра в 10 позвонить в банк")],
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const joined = texts(sent).join("\n").toLowerCase();
      if (!/напомн|10:00|в 10/i.test(joined)) return "на прямую просьбу поставить напоминание Джарвис не подтвердил действие";
      return null;
    },
  },
  {
    // Живой случай нового человека: наговорил минуту в знакомстве, бот молча
    // задал следующий вопрос, а потом сказал «голосовых у тебя 0». Проверяем,
    // что ответ в знакомстве вообще получает отклик по существу.
    name: "Знакомство: отвечает на сказанное, а не молчит",
    heavy: true,
    chain: () => [
      message("давай познакомимся"),
      message("мне сейчас тяжело, недавно развёлся и никак не приду в себя"),
      message("/stop"),
    ],
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const joined = texts(sent).join("\n");
      if (joined.trim().length < 40) return "знакомство не ответило на рассказ человека";
      return null;
    },
  },
  {
    // «Хочу в любой момент обратиться, чтобы он думал и был в контексте
    // разговора». Проверяем оба: что зов по имени вообще работает и что агент
    // помнит сказанное ДВЕ реплики назад, а не отвечает с чистого листа.
    name: "Джарвис: помнит, о чём только что говорили",
    heavy: true,
    chain: () => [
      message("сегодня возил младшую к врачу, сказали всё в порядке"),
      message("джарвис, к кому я сегодня возил младшую и что сказали?"),
    ],
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const joined = texts(sent).join("\n").toLowerCase();
      if (!/врач|доктор|педиатр|поликлин/i.test(joined)) {
        return "на прямой вопрос про только что рассказанное агент не вспомнил врача — общая память разговора не работает";
      }
      return null;
    },
  },
  ...
  },
  {
    ...
    chain: () => [
      message("помоги зафиксировать день"),
      callback("day:len:6"),
      message("сегодня дописал большую часть нового агента, провозился до ночи, но наконец заработало и я очень доволен"),
      message("/stop"),
    ],
    check: (sent) => {
      const broken = notBroken(sent);
      ...
      if (!last) return "бот не задал следующий вопрос";
      if (/по дому|хозяйств|покуп|потратил|магазин/i.test(last)) {
        return `на живой ответ бот ушёл в быт: «${last.slice(0, 90)}» — за интересное надо цепляться`;
      }
      return null;
    },
  },
  {
    ...
    name: "Разбор дня: не застревает на одной теме",
    heavy: true,
    chain: () => [
      message("помоги зафиксировать день"),
      callback("day:len:6"),
      message("был в булочной и в био магазине, купил помидоры и инжир"),
      message("да просто запасался, ничего особенного"),
    ...
    check: (sent) => {
      ...
      if (broken) return broken;
      const qs = texts(sent).filter((t) => /\d\/\d/.test(t));
      if (qs.length < 3) return "бот не дошёл до третьего вопроса — разбор дня оборвался";
      // Один шаг вглубь после содержательного ответа — это правильно, за живое
      // надо цепляться. А вот когда и после «ничего особенного» всё ещё та же
      // тема — это рут: ветка кончилась, а бот её не отпускает.
      const food = /магазин|купил|продукт|еда|ел\b|блюдо|булочн|помидор|инжир|запас/i;
      const later = qs.slice(1);
      if (later.every((q) => food.test(q))) {
        return `все вопросы после первого про еду и покупки: «${later[later.length - 1].slice(0, 90)}» — разбор застрял`;
      }
      return null;
    },
  },
  ...
  {
    // Случай Игоря 26.08: агент сказал «Собрал постановку — вот что получилось»,
    // а саму постановку не прислал (отдельное сообщение молча отверг Telegram).
    // Теперь реплика и постановка склеены в одно сообщение — проверяем, что
    // финал разговора действительно ПОКАЗЫВАЕТ постановку и кнопку отправки.
    name: "Идея: финал показывает постановку с кнопкой",
    heavy: true,
    chain: () => [
      message("идея по life os: пусть фото с геолокацией ложатся точками на карту, чтобы видеть маршрут жизни"),
      message("да, всё верно, больше нечего добавить — собирай и отправляй постановку"),
    ],
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const withPreview = sent.find((s) => /вот как я это понял/i.test(String(s.text || "")));
      if (!withPreview) {
        ...
        return promised ? "бот сказал «собрал постановку», но саму постановку не показал" : null;
      }
  ...
  {
    name: "Ссылку на карту бот открывает, а не переспрашивает город",
    heavy: true,
    // Живой случай: «вот тут мы будем жить, найди что интересного вокруг» со
    // ссылкой на карту. Бот отвечал «я не хожу по ссылкам» и предлагал кнопками
    // Париж и Барселону — хотя место написано прямо в адресе ссылки.
    send: () =>
      message(
        "вот тут мы будем жить https://www.google.com/maps/place/Snorrabraut+29,+105+Reykjav%C3%ADk,+Iceland — найди что интересного есть вокруг",
      ),
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const joined = texts(sent).join("\n");
      if (/не хожу по ссылк|не могу открыть ссылк|перейти по ссылк.{0,20}не/i.test(joined)) {
        return "бот отказался открывать ссылку";
      }
  ...
  },
  {
    ...
    // ставил напоминание на выдуманное время. Выдуманное напоминание хуже
    // вопроса: срабатывает, когда не ждут, и молчит, когда надо.
    chain: () => [
      message("напомни купить молоко для самопроверки"),
      message("завтра в 9:00"),
    ],
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const all = texts(sent).join("\n");
      if (!/когда|when|коли/i.test(all)) return "бот не спросил «когда?» — поставил напоминание сам";
      const last = texts(sent).slice(-1).join("\n");
      return /напомню|remind|нагадаю/i.test(last) && /9:00|09:00/.test(last)
        ? null
        : `после ответа «завтра в 9:00» напоминание не встало: «${last.slice(0, 120)}»`;
    },
  },
  {
  ...
  {
    name: "Размышление не тянет за собой сканы документов",
    heavy: true,
    // Живой случай: «напомни, какая у меня была мысль про то, что душе не нужна
    // еда» — и бот прислал чужой пост из соцсети и техпаспорт машины с VIN.
    // Поиск зацепил их по касательной. Скан документа в ответ на размышление —
    // это не услужливость, а утечка: человек его не просил.
    send: () => message("напомни, какая у меня была мысль про то, что душе не нужна еда"),
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const files = sent.filter((s) => /sendPhoto|sendDocument|sendVideo|sendMediaGroup/.test(s.method));
      return files.length ? `к размышлению приложено файлов: ${files.length}` : null;
    },
  },
  {
    name: "«Дай паспорт» — наоборот, присылает сам документ",
    heavy: true,
    // Обратная сторона того же правила: когда документ ПРОСЯТ, он должен прийти
    // файлом, а не пересказом содержимого.
    chain: () => [
      message("запиши: скан страховки на машину для самопроверки, номер полиса 7788"),
      message("покажи мою страховку на машину для самопроверки"),
    ],
    check: (sent) => notBroken(sent),
  },
  {
    name: "Пусто в архиве — бот отвечает сам, а не предлагает спросить",
    heavy: true,
    // «Найди что интересного вокруг» бот искал в СВОИХ ЖЕ сохранёнках, не нашёл
    // и предложил: «просто скажи, и я расскажу». Человек сказал — бот предложил
    // ещё раз. Предлагать то, что можешь сделать сейчас, — это отговорка.
    send: () => message("что интересного посмотреть в Рейкьявике рядом с улицей Snorrabraut?"),
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const joined = texts(sent).join("\n");
      if (/просто скажи|просто спроси|могу ответить на это сам|спроси меня как ai/i.test(joined)) {
        return "бот предложил спросить его вместо ответа";
      }
      return joined.length > 120 ? null : "вместо ответа по существу — отписка";
    },
  },
  {
    name: "Ответ на свой же уточняющий вопрос не уходит в дневник",
    heavy: true,
    // Бот спросил «в каком городе?», человек написал словами — и получил
    // «звучит как что-то важное, но я не совсем понимаю, что ты имеешь в виду».
    chain: () => [
      message("найди для самопроверки, что интересного посмотреть вокруг"),
      message("Рейк явик"),
    ],
    check: (sent) => {
      const broken = notBroken(sent);
      if (broken) return broken;
      const last = texts(sent).slice(-1).join("\n");
      return /не совсем понимаю|не понял, что ты имеешь в виду|звучит как что-то важное/i.test(last)
        ? "бот не связал ответ со своим же вопросом"
        : null;
    },
  },
  {
  ...
  },
  {
    ...
    heavy: true,
    // Все три шага в одном сценарии. Раньше «отмени последнее» опиралось на то,
    // что задачу удалил СОСЕДНИЙ сценарий, — и любой новый сценарий, вставший
    // между ними, подменял «последнее»: отмена откатывала чужое действие.
    chain: () => [
      message(`добавь задачу ${TASK_MARK}`),
      message(`убери задачу ${TASK_MARK}`),
      message("отмени последнее, верни как было"),
    ],
    check: (sent) => notBroken(sent),
...
const EMPTY_SEARCH_CASE: QualityCase = {
  ...
  expect: "Бот честно говорит, что записей про отпуск не нашёл. Сказать «записей нет» или «дневник пока пуст» — НОРМАЛЬНО: счётчики записей бот действительно видит. Провал — только выдуманные ДЕТАЛИ, которых он знать не может: дата или давность регистрации («ты недавно зарегистрировался»), пересказ конкретных записей или напоминаний, которых не существует.",
};
