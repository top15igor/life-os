...
import { rulesHint, rulesBlock } from "./sortShelf";
...
// Личные правила человека кладём в ОПИСАНИЕ поля «категория» — там, где
// модель принимает решение. Одного упоминания в задании оказалось мало.
function toolWith(list: string): Anthropic.Tool {
  if (!list) return TOOL;
  const t: any = JSON.parse(JSON.stringify(TOOL));
  t.input_schema.properties.category.description += ` ЛИЧНЫЕ ПРАВИЛА ЭТОГО ЧЕЛОВЕКА (важнее всего выше): ${list}.`;
...
export async function analyzeImage(base64: string, mediaType: string, userId?: string): Promise<VisionResult> {
  ...
  const hint = userId ? await rulesHint(userId) : "";
  const prompt = `Ты — «Визуальная память» дневника LIFE OS. Посмотри на изображение и опиши его СМЫСЛ для человека (НЕ технический OCR, не сырой текст). Заполни describe_image: category, человеческий title, короткий summary, важные fields (только реально видимые данные — не выдумывай), date если видна, confidence по чёткости. Если фото нечёткое или непонятное — confidence: low. ВАЖНО про category: скриншот поста соцсети, переписки, приложения, мема, видео или веб-страницы — это НЕ «moment» (moment только для реальных личных фото из жизни). Такой скриншот относи к info (полезная информация) или document, но не к moment. ВАЖНО: если это удостоверение личности, паспорт, виза, права, гарантия или страховка и виден СРОК ДЕЙСТВИЯ (на укр. «Дата закінчення строку дії», Date of expiry, «действителен до», «valid until») — ОБЯЗАТЕЛЬНО добавь отдельное поле с label «Действителен до» и value датой в формате ГГГГ-ММ-ДД (сконвертируй из любого вида, напр. «24 ЖОВ/OCT 27» → 2027-10-24). Не путай с датой выдачи.${rulesBlock(hint)}`;
  const m = await new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY }).messages.create({
    model: "claude-sonnet-4-6",
    max_tokens: 1200,
    tools: [toolWith(hint)],
