...
const DRAFT_HINT: Record<Lang, string> = {
  ru: "\n\nСохранить как запись за сегодня? Можно дописать — просто пришли ещё пару слов, я вплету их в текст.",
  en: "\n\nSave it as today's entry? You can add more — just send a few more words and I'll weave them in.",
  uk: "\n\nЗберегти як запис за сьогодні? Можна дописати — просто надішли ще пару слів, я вплету їх у текст.",
...
async function nextQuestion(userId: string, lang: Lang, ctx: DayCtx, asked: string[], answers: string[], max: number): Promise<{ q: string; chips: string[] }> {
  ...
  const AREAS = [
    ...
    "перемещения и места — где был, куда ходил и ездил",
    "тело и самочувствие — сон, еда, спорт, усталость",
    "деньги и покупки — на что потратил, что купил",
  ...
  const stage = isFirst
    ...
    : isLast
      ...
      : `СЕРЕДИНА: открой НОВУЮ СТОРОНУ дня. Посмотри, о чём уже спрашивал, и выбери ту, которой ещё НЕ касался:\n${AREAS.map((x) => `— ${x}`).join("\n")}\nУглубляться в предыдущий ответ НЕЛЬЗЯ, даже если он интересный: один вопрос — одна сторона дня. Иначе весь разбор превратится в допрос про один поход в магазин, а день останется нерассказанным.`;
...
async function replyToFeedback(userId: string, lang: Lang, text: string, asked: string[]): Promise<string> {
  ...
  try {
    const m = await client().messages.create({
      ...
      max_tokens: 300,
      temperature: 0.4,
      system: `Человек разбирает свой день с ботом и сделал замечание о том, КАК бот это делает. Ответь ему коротко, 1-3 предложения, на языке ${LANG_NAME[lang]}.\n\nПРАВИЛА: согласись, если он прав, и скажи это прямо — без «извини» и без обещаний «я исправлюсь», ты не можешь себя переписать. Скажи, что можно сделать ПРЯМО СЕЙЧАС: задать вопрос про другую сторону дня, дописать черновик своими словами или сохранить как есть. И предложи оформить это как идею по продукту, если замечание того стоит.`,
...
export async function dayAnswer(userId: string, lang: string, answer: string): Promise<DayTurn> {
  ...
  if (st.draft) {
    if (await looksLikeFeedback(userId, a)) {
      ...
      return { text: `${said}\n\n${DRAFT_LEAD[l]}\n\n${st.draft}${DRAFT_HINT[l]}`, phase: "draft" };
    }
