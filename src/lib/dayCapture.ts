...
async function nextQuestion(userId: string, lang: Lang, ctx: DayCtx, asked: string[], answers: string[], max: number): Promise<{ q: string; chips: string[] }> {
  ...
  const stage = isFirst
    ...
    : isLast
      ...
      : `СЕРЕДИНА: спроси ПРО ЭТО и ни про что другое — «${AREAS[(n - 1 + new Date().getDate()) % AREAS.length]}».\n\nЖЁСТКОЕ ПРАВИЛО: в вопросе не должно быть НИ ОДНОГО слова из того, что он уже рассказал. Не переспрашивай и не уточняй сказанное — спрашивай про то, о чём он ещё НЕ говорил. Если названная сторона дня у него сегодня явно не случилась, спроси про неё же в самом простом виде («а <область> сегодня как?»), но НЕ возвращайся к прошлым темам.`;
