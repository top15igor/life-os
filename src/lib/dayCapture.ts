...
async function nextQuestion(userId: string, lang: Lang, ctx: DayCtx, asked: string[], answers: string[], max: number): Promise<{ q: string; chips: string[] }> {
  ...
  const stage = isFirst
    ...
    : isLast
      ...
      : "СЕРЕДИНА: про главное событие дня или про то, за что зацепился его предыдущий ответ.";
...
— Если в контексте ниже есть зацепка (планы на сегодня, обещание, шаги, траты) — спроси про НЕЁ конкретно, по имени и цифре. Это ценнее общего вопроса.
...
export async function dayAnswer(userId: string, lang: string, answer: string): Promise<DayTurn> {
  ...
  // Фаза черновика: любой текст — это дополнение, вплетаем и пересобираем.
  if (st.draft) {
