...
// Сколько полных дней прошло между двумя датами YYYY-MM-DD (по календарю, без времени).
...
// Человеческая метка давности для промпта: модели плохо считают даты,
// поэтому «сколько дней назад» вычисляем кодом и отдаём готовым.
...
export async function personalMorning(
  ...
  try {
    ...
    // Дата и день недели — по часовому поясу пользователя, не по UTC сервера.
    const lp = localParts(p.tz, new Date());
    const todayKey = lp.dateKey;
    const weekdayRu = WEEKDAY_RU[lp.weekday] || "";
    // Давность последней записи считаем кодом: именно на этом бот «терялся в днях»
    // и называл события недельной давности «вчера».
    const lastEntryDays = entries.length ? daysBetween(entries[0].entry_date, todayKey) : null;
    const stale = lastEntryDays !== null && lastEntryDays >= 3;

    const ctx: string[] = [`Имя: ${name || "—"}`, `Серия дней подряд: ${streak}`];
    if (lastEntryDays !== null) ctx.push(`Последняя запись пользователя: ${agoLabel(lastEntryDays)}.`);
    if (effective.includes("diary"))
      ctx.push("Последние записи (давность — дата: суть):\n" + entries.map((e: any) => `${agoLabel(daysBetween(e.entry_date, todayKey))} — ${e.entry_date}: ${(e.summary || e.raw_text || "").slice(0, 300)}`).join("\n"));
    ...
    // а даём «искру дня»: короткий вопрос/фразу, которая заводит на день (просьба Коли).
    const sparkLine = stale
      ? `

ВАЖНО, ОСОБЫЙ СЛУЧАЙ: пользователь не писал уже ${lastEntryDays} дн. НЕ делай вид, что вы общались вчера, НЕ спрашивай «как прошло» про старые события и НЕ пересказывай старые записи как свежие. Вместо этого дай «искру дня» — одну тёплую фразу или вопрос, который откликается внутри и заводит на день. Примеры духа (не повторяй дословно, придумай своё в манере пользователя): «Интересно, какое неожиданное везение ждёт тебя сегодня?», «Какое одно маленькое дело сегодня — то, за которое будущий ты скажет спасибо?», «Если сегодня случится что-то классное — где бы ты хотел, чтобы оно тебя нашло?». НЕ называй, сколько дней он не писал, не начинай с паузы и не зови «вернуться» — никакого укора, просто тёплое утро; в самом конце можно легко спросить, как у него дела.`
      : "";
    const prompt = `Сейчас утро, сегодня ${today}, ${weekdayRu}. Составь ОДНО личное утреннее сообщение для пользователя.
