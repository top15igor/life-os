...
export const ACTION_TOOLS: any[] = [
  ...
  },
  {
    name: "ask_clarify",
    description:
      "ПЕРЕСПРОСИТЬ, когда просьба понятна по смыслу, но в ней не хватает главного. Два случая. ПЕРВЫЙ: команда записать/сохранить БЕЗ самого содержимого — «запиши мне», «давай запишем», «записывай всё», «запиши мне настроение» (а какое настроение — не сказано): спроси «что именно записать?», options НЕ заполняй. НЕ отвечай «Записал!» на пустоту — записывать нечего. ВТОРОЙ: неоднозначно ИМЕННО В ТОМ, что нужно менять или удалять, и ошибка будет стоить дорого. Примеры: «удали запись про Колю», а Коль в дневнике двое; «перенеси встречу», а встреч на неделе три; «убери ту трату», а трат с такой суммой несколько.\n\nquestion — один короткий вопрос. options — 2-4 варианта короткими подписями для кнопок.\n\nНЕ злоупотребляй: если понятно из последней реплики или из недавнего разговора — действуй, не переспрашивай. Переспрос уместен только там, где неверная догадка ПОРТИТ ДАННЫЕ. Для поиска и вопросов переспрашивать не нужно — покажи, что нашёл.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "just_ack",
    description:
      "Реплика-СВЯЗКА, после которой ничего не надо делать и НИЧЕГО не надо сохранять: «спасибо», «ок», «понял», «супер», «забей», «проехали», «не надо», «забудь», «всё, отбой». Человек закрывает тему или благодарит — он НЕ диктует запись. reply — твой короткий живой ответ одной строкой (например «Ок, ничего не записываю 👍» на «забей» или «Всегда пожалуйста 🙂» на «спасибо»). ВАЖНО: если в реплике есть хоть какое-то СОДЕРЖАНИЕ о жизни (события, чувства, факты) — это save_entry, а не связка. Сомневаешься — save_entry.",
    input_schema: { type: "object", properties: { reply: { type: "string", description: "короткий ответ одной строкой, без обещаний что-то сделать" } }, required: ["reply"] },
  },
  {
    name: "cannot_do",
    description:
      "Человек просит СДЕЛАТЬ во внешнем мире то, чего бот не умеет: заказать такси или еду, купить что-то, позвонить, отправить письмо, забронировать, включить музыку, перевести деньги. Выбирай ТОЛЬКО когда у просьбы есть ПОСЛЕДСТВИЕ в мире (что-то купится, отправится, приедет). НАЙТИ, РАССКАЗАТЬ, ПОСОВЕТОВАТЬ, ПОСМОТРЕТЬ — «найди что интересного вокруг», «что посмотреть в городе», «расскажи про…», «посоветуй кафе» — это НЕ cannot_do, а ask_question: у ассистента есть веб-поиск, он умеет отвечать про мир. РАССКАЗ о событии («вызвал такси», «купил хлеб») — save_entry. what — суть просьбы 2-4 словами, как сказал человек.",
  ...
  },
  {
    name: "set_pushes",
    description:
      "Включить, выключить или перенести СООБЩЕНИЯ БОТА ПО РАСПИСАНИЮ. Примеры: «пиши мне каждое утро», «пиши в 7 утра», «не пиши по утрам», «не пиши мне пока», «не трогай меня по выходным», «не присылай вечерние вопросы», «хватит напоминать записать день», «не шли недельный итог», «включи всё обратно», «включи/отключи напоминания» (без уточнения какие — это про рассылку). ВАЖНО: «напоминай мне каждый день в 9 утра» БЕЗ предмета (не сказано, ЧТО напоминать) — это тоже сюда, morning=true + morning_hour: человек просит ежедневное утреннее сообщение. С предметом («напоминай пить воду каждый день») — это set_reminder с recurrence. Это НЕ set_style: тот меняет МАНЕРУ речи, а здесь включается и выключается сама отправка. Заполняй только те поля, о которых человек сказал.",
    input_schema: {
      type: "object",
      properties: {
...
const SYS =
  ...
  "СВЯЗКИ И ОТМЕНЫ: «спасибо», «ок», «понял», «забей (на это)», «проехали», «не надо», «забудь» — это just_ack: ничего не делать и НЕ сохранять. Команда записать БЕЗ содержимого («запиши мне», «давай запишем», «записывай всё») — ask_clarify с вопросом «что именно записать?» без options: отвечать «Записал!» на пустоту — обман.";
...
export async function runAction(userId: string, name: string, input: any, lang: Lang, tzOffset?: number | null): Promise<ActionResult> {
  ...
  try {
    ...
    if (name === "just_ack") {
      // «Спасибо», «забей», «проехали» — не запись и не команда. Раньше боту
      // было некуда деться: tool_choice=any заставлял выбрать хоть что-то, и
      // «забей на это» сохранялось записью с бодрым «Записал!» — то есть бот
      // делал ровно противоположное сказанному и врал об этом.
      const r = String(input?.reply || "").replace(/\s+/g, " ").trim().slice(0, 200);
      return { text: r || "Ок 👍" };
    }
    if (name === "ask_clarify") {
      ...
      if (!q) return { text: s.fail };
      // Вариантов может и не быть: на «запиши мне» честный вопрос — «что именно
      // записать?», а варианты тут не из чего собрать. Ответ словами свяжет
      // механика уточнений.
      if (opts.length < 2) return { text: q };
