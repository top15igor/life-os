...
export const ACTION_TOOLS: any[] = [
  ...
  },
  {
    name: "resume_idea",
    description:
      "Человек хочет ВЕРНУТЬСЯ к идее по LIFE OS, которую уже предлагал раньше: «давай вернёмся к идее про лендинг», «помнишь идею про напоминания — я передумал», «доработаем идею 7», «хочу переделать то, что предлагал вчера». query — по каким словам или номеру её искать.\n\nЭто НЕ propose_idea (та для НОВОГО предложения) и не поиск по дневнику.",
    input_schema: { type: "object", properties: { query: { type: "string" } }, required: ["query"] },
  },
  {
    name: "ask_clarify",
