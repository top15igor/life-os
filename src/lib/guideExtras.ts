...
const E: Record<Locale, Extras> = {
  ru: {
    ...
    changelog: [
      { t: "Карта жизни: видео, наведение порядка и плавный ход", d: "Первая карта умела только показывать. Теперь ею можно управлять. На карту принимаются ВИДЕО: ролик хранит место и время съёмки внутри себя, и Telegram их не вырезает — так что видео, присланное боту обычным способом, встаёт точкой само (фото по-прежнему нужно слать файлом). Кадр-обложку для ролика снимает браузер при загрузке, поэтому на карте видно не серый квадрат, а сам момент; нажал на точку — ролик играет прямо в карточке. Кнопка «Добавить фото или видео» прямо над картой: выбираешь файлы с компьютера или телефона — они уходят в «Память», а те, у кого координаты есть, сразу становятся точками; у кого нет — карта тут же предлагает ткнуть место руками. У каждой точки появились действия: приблизить, ПЕРЕСТАВИТЬ (ткнул в новое место — точка переехала), УБРАТЬ С КАРТЫ (снимок цел, просто вернулся в список «без точки») и УДАЛИТЬ насовсем — с вопросом, потому что это уносит фото и из «Памяти», и из хранилища. Рядом ссылка «Открыть в Google Картах» и кнопка «во весь экран». И главное на ощупь: карта перестала прыгать ступенями — масштаб теперь дробный и тянется плавно, а переходы к точке и к выбранному году идут полётом, а не рывком. Ещё: в списке «без точки» больше нет чеков, договоров и скриншотов — на карту просятся моменты, а не бумаги.", tag: "improved" },
    ...
    features: [
      {
        ...
        sections: [
          ...
          { h: "Видео", p: "Ролики карта принимает наравне с фото — и это тот редкий случай, где всё проще: место и время съёмки видео хранит внутри себя, и Telegram их не стирает. Значит, ролик, присланный боту обычным способом, встаёт на карту сам. Обложку кадром снимает браузер при загрузке с сайта, а само видео играет прямо в карточке точки. Через бота принимаю ролики до 20 МБ (предел Telegram), через сайт — до 45 МБ." },
    ...
    features: [
      {
        ...
        sections: [
          ...
          { h: "Video", p: "The map takes clips just like photos — and this is the rare case where things are simpler: a video keeps the place and time of shooting inside itself, and Telegram does not wipe it. So a clip sent to the bot the ordinary way lands on the map by itself. The cover frame is grabbed by your browser on upload, and the video plays right in the point's card. Via the bot: clips up to 20 MB (Telegram's limit); via the site: up to 45 MB." },
          { h: "Tidying up", steps: [
