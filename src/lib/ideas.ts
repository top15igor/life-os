...
import { appCheatsheet } from "./accountFacts";
...
ЧТО ТАКОЕ LIFE OS. Личный архив жизни: Telegram-бот и сайт. Человек надиктовывает боту события дня, AI разбирает их и раскладывает по разделам сам.

ПОЛНЫЙ СПИСОК ТОГО, ЧТО УЖЕ РАБОТАЕТ, — ниже. Знай его и опирайся на него: половина предложений оказывается уже сделанной, и честнее сказать об этом сразу, показав, где это лежит, чем обсуждать как новое.
...
export async function converse(userId: string, text: string): Promise<Turn> {
  ...
  try {
    // Шпаргалка по возможностям продукта — та же, что у AI-друга. Коля прямо
    // об этом попросил: «агент, который обсуждает доработки, должен хорошо
    // знать, что уже есть и как это работает».
    const know = appCheatsheet("ru");
    const m = await new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY }).messages.create({
      model: "claude-sonnet-4-6",
      max_tokens: 900,
      temperature: 0.5,
      system: [
        { type: "text" as const, text: `${SYS}\n\n${know}`, cache_control: { type: "ephemeral" as const } },
      ],
      tools: [FINISH],
