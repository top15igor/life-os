...
const GUIDES: Record<Locale, Record<string, PageGuide>> = {
  ru: {
    ...
    "/knowledge": {
      can: [
        ...
        "Кинь боту ссылку Instagram/YouTube/TikTok/Facebook — AI сохранит суть сюда (не в дневник).",
  ...
  en: {
    ...
    "/knowledge": {
      can: ["Your personal knowledge base from the web.", "Send the bot an Instagram/YouTube/TikTok/Facebook link — AI saves the gist here (not to the diary).", "A title, key points and tags for each saved item."],
...
- База знаний (/knowledge): сохранёнки из интернета. Кинь боту ссылку Instagram/YouTube/TikTok/Facebook — AI сохранит суть (заголовок, тезисы, теги) сюда, а НЕ в дневник.
...
- Ссылка Instagram/YouTube/TikTok/Facebook → в Базу знаний. Ссылка товара или «/wish <ссылка>» → в Вишлист. Фото обложки книги с подписью «книга» → в Книги. Фото/документ → в Память.
...
ВАЖНОЕ ПРАВИЛО ДЛЯ ОТВЕТОВ: Если пользователь просит сделать то, что выходит за возможности этого приложения (заказать такси, позвонить, отправить письмо, управлять внешними сервисами и т.п.) — СНАЧАЛА честно и прямо скажи, что это не умеешь. Не делай вид, что выполнил запрос, не молчи об ограничении. После честного «я так не умею» — предложи то, что реально можешь сделать (записать напоминание, добавить в цели, сохранить мысль и т.д.). Обман пользователя хуже, чем честный отказ.`;
