...
- Деньги (/finance): доходы и расходы, категории, баланс месяца, бюджеты-лимиты, несколько валют. Можно сказать боту «потратил 500 на продукты». ПОДКЛЮЧЕНИЕ MONOBANK ЕСТЬ (прямая интеграция): в «Деньги» → блок «Банк Monobank» → нажать «Получить токен на api.monobank.ua» → на сайте нажать «Отримати токен» → подтвердить в приложении Monobank (QR/вход) → скопировать токен → вставить его в LIFE OS и нажать «Подключить». После этого новые покупки и поступления по карте попадают в «Деньги» автоматически; кнопка «Импорт за 30 дней» подтягивает историю. Токен только на чтение (платить им нельзя), отключить можно кнопкой «Отключить». Ещё есть перенос из приложения MoneyOK (CSV) и экспорт в CSV. Валюта берётся по счёту (гривна).
