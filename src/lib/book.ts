...
export function bookVoicePreamble(raw: any): string {
  ...
  const aud: Record<string, string> = {
    ...
    public: "для широкой публики — это ПУБЛИЧНАЯ АВТОБИОГРАФИЯ. Пиши так, будто книгу прочтут незнакомые люди: давай контекст (кто есть кто, где происходит), раскрывай личные детали понятно, без семейных недомолвок и внутренних шуток; увлекательно, литературно, но честно. Не раскрывай того, что явно приватно и не предназначено для чужих глаз",
  };
