...
// Всё, что бот УЖЕ знает о человеке: упомянутые люди + релевантные по смыслу записи
// (pgvector) + недавние записи дневника. Чтобы в знакомстве бот НЕ переспрашивал
// уже известное (кто такой Вовчик, кем работает, где живёт и т.п.).
async function knownAbout(userId: string, query: string): Promise<string> {
  const db = supabaseAdmin();
  const [recall, entRes, pplRes] = await Promise.all([
    recallContext(userId, query || "факты о человеке, его близкие, работа, жизнь").catch(() => ""),
    db.from("entries").select("entry_date, summary, raw_text").eq("user_id", userId).order("entry_date", { ascending: false }).limit(40),
    db.from("people").select("name").eq("user_id", userId).eq("hidden", false).limit(80),
  ]);
  const people = (((pplRes as any).data as any[]) || []).map((p) => (p?.name || "").trim()).filter(Boolean);
  const recent = (((entRes as any).data as any[]) || []).map((e) => `${e.entry_date}: ${(e.raw_text || e.summary || "").slice(0, 300)}`).join("\n");
  const parts: string[] = [];
  if (people.length) parts.push(`ЛЮДИ, КОТОРЫХ ЧЕЛОВЕК УЖЕ УПОМИНАЛ (ты их знаешь — НЕ спрашивай «а кто это»): ${people.join(", ")}`);
  if (recall) parts.push(recall);
  if (recent) parts.push(`НЕДАВНИЕ ЗАПИСИ ДНЕВНИКА (это ты уже знаешь про него):\n${recent}`);
  return parts.join("\n\n") || "(пока почти ничего не известно — это самое начало)";
}
...
ЧТО Я УЖЕ ЗНАЮ О ЧЕЛОВЕКЕ (не переспрашивай это, опирайся на это):
${known}
...
ТЕМЫ ДЛЯ ВОПРОСОВ (запасные — только если ниточки в ответах и знании нет; следующая неиспользованная):
${Q_THEMES.map((q, i) => `${i + 1}. ${q}`).join("\n")}

ИСТОРИЯ ДИАЛОГА (не повторяйся):
${history}
