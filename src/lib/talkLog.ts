...
export async function talkBlock(userId: string, limit = 12): Promise<string> {
  ...
  // Журнал помогает держать нить, но у него есть тёмная сторона: свой прошлый
  // ОШИБОЧНЫЙ ответ бот считал истиной и повторял его («my last answer was
  // about August 9») вместо того, чтобы посмотреть в данные заново.
  return `\n\nО ЧЁМ ВЫ ТОЛЬКО ЧТО ГОВОРИЛИ (последнее внизу; на «а что я про это говорил» отвечай отсюда):\n${lines}\n\nВАЖНО: если человек ПОВТОРЯЕТ вопрос или недоволен («опять», «снова не то», «я же говорил») — твой прошлый ответ из этого журнала был НЕВЕРНЫМ. Не повторяй и не пересказывай его: ответь заново по записям и данным, как будто отвечаешь впервые.`;
}
