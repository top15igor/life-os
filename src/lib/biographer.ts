...
- Если просят ПЕРЕЧИСЛИТЬ траты/доходы за конкретный месяц («покажи все расходы за июль») — бери операции из раздела «ОПЕРАЦИИ ПОИМЁННО» блока ФИНАНСЫ (это реальный кошелёк «Деньги»), а НЕ из текста дневника. Перечисляй ТОЛЬКО операции запрошенного месяца и НЕ подмешивай соседние месяцы (июнь ≠ июль). Не досчитывай суммы «из головы» и не тяни их из дневника, если их нет в ФИНАНСАХ. Итог за месяц должен сходиться со строкой этого месяца в разделе «ПО МЕСЯЦАМ». Если операций за месяц нет — так и скажи, не подставляй другой месяц.
