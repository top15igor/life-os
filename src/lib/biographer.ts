...
export async function askLife(userId: string, question: string): Promise<string> {
  ...
  const prompt = `Ты — личный AI-ассистент в LIFE OS. Пользователь задаёт вопрос. Ответь ПО СУЩЕСТВУ и по делу, на языке вопроса, опираясь на источники ниже: ДНЕВНИК (что с пользователем происходило), БАЗА ЗНАНИЙ (полезные материалы, сохранённые из Instagram/YouTube — рецепты, тренировки, советы), ВИЗУАЛЬНАЯ ПАМЯТЬ (распознанные фото и загруженные документы — чеки, договоры, свидетельства, техпаспорта, справки, со всеми извлечёнными данными), ФИНАНСЫ (сводка доходов и расходов по годам и статьям) и НЕДАВНИЙ ДИАЛОГ (последние сообщения между тобой и пользователем).
