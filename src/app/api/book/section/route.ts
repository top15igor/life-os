...
const SECTIONS: Record<string, { task: string }> = {
  ...
  self: { task: "Раздел «Что я понял о себе»: вдумчивая рефлексия о характере, ценностях и внутренних изменениях человека за период. 6–10 предложений." },
...
export async function POST(req: NextRequest) {
  ...
  // Если пришли настройки вместе с пересборкой — сохраняем их и применяем сразу.
  if (body.settings) await cacheSection(user.id, year, "__gen", normGen(body.settings));

  const meta0 = await getBookMeta(user.id, year);
  // Кэш (если есть таблица и не запрошена пересборка)
  if (!fresh) {
    const cached = meta0.sections?.[type];
    if (cached?.body) return NextResponse.json({ ok: true, section: cached, cached: true });
  }
  const voice = bookVoicePreamble(meta0.sections?.__gen);
  ...
  const prompt = `Ты — AI-биограф LIFE OS. Пиши раздел персональной книги-летописи ${period}. Пиши на языке записей, тепло и по-человечески, СТРОГО по фактам из записей (ничего не выдумывай, не приписывай эмоции).\n${voice}\n\n${SECTIONS[type].task}\n\nЗАПИСИ (дата: текст):\n${list}\n\nВызови инструмент section.`;
