...
export async function POST(req: NextRequest) {
  ...
  if (owner) {
    const label = kind === "gap" ? "🙋 Бот не умеет" : kind === "idea" ? "💡 Идея" : kind === "bug" ? "🐞 Проблема" : kind === "sync" ? "🔗 Синхронизация / интеграция" : kind === "wish" ? "✨ Пожелание" : kind === "book_order" ? "📖 Заявка на КНИГУ" : kind === "plan_order" ? "⭐️ Заявка на ТАРИФ" : "💬 Отзыв";
    try {
