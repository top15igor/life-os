...
export default async function HealthPage() {
  ...
  const healthEntries = entries.filter((e: Entry) => cats(e).some((c: any) => ["health", "sport", "food"].includes(c.slug)));
    ...
    <div className="shell">
      ...
      <main className="main">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 9, marginBottom: 20 }}>
          ...
          <Metric label={s.sleep} icon="ti-moon" value={latest(entries, "sleep_hours")} suffix={s.su} color="#4f46e5" />
          <Metric label={s.weight} icon="ti-scale" value={latest(entries, "weight")} suffix={s.wu} color="#0ea5e9" />
        </div>
