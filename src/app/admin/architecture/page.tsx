...
const TOC: { id: string; label: string }[] = [
  ...
  { id: "backups", label: "Резервные копии и логи" },
...
const SECURITY = [
  ...
  "Данные шифруются в покое и передаются по защищённому каналу (TLS); резервные копии — см. раздел «Резервные копии и логи».",
...
const BACKUPS = [
  ["Вся база · еженедельно", "Каждое воскресенье крон собирает дамп ВСЕХ таблиц (все пользователи) в .zip с JSON-файлами и шлёт владельцу в Telegram. Ручной запуск: открыть /api/admin/dbdump (только владелец). Восстановление — вставить JSON обратно в Supabase.", "Данные людей не зависят от одного сервиса"],
  ["Личный дневник · ежемесячно", "1-го числа каждый пользователь получает свой дневник в .zip (Obsidian-vault, Markdown) — можно выключить в Профиле.", "«Твои данные всегда у тебя»"],
  ["Экспорт в один клик", "Любой пользователь в любой момент скачивает все свои данные (/api/export).", "Гарантия из /about"],
  ["Supabase-бэкапы", "На бесплатном тарифе провайдер копий НЕ делает; ежедневные копии — на платном Pro. Наш недельный дамп закрывает этот пробел.", "Проверить тариф при росте"],
];

const OPS_NOTES = [
  ["Логи прода", "Vercel хранит логи ~1 час (бесплатный тариф). Ошибку пользователя ловим «по горячему»: повторить действие и сразу смотреть vercel logs."],
  ["Секретные ключи", "Все ключи (Anthropic, OpenAI, Telegram, Supabase) — в переменных Vercel с пометкой Sensitive: наружу не выгружаются даже владельцем, только замена новыми. В коде ключей нет."],
  ["Бот-справка владельца", "Владелец может спросить бота о любом из этого («где логи?», «как устроены бэкапы?») — у ассистента и AI-друга есть эта справка, видна только владельцу."],
];
  ...
  return (
    <div className="shell">
      ...
      <main className="main">
        ...
        <Sec id="backups" icon="ti-database-export" title="Резервные копии и логи" sub="Данные живут только в Supabase — поэтому копии делаем сами, по расписанию. Плюс операционные заметки: логи и ключи.">
          <Table head={["Копия", "Как работает", "Зачем"]} rows={BACKUPS} />
          <Table head={["Заметка", "Суть"]} rows={OPS_NOTES} />
        </Sec>
