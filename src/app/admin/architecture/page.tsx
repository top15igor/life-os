...
// «Может ли кто-то удалить/испортить?» — разобрано по фактам после открытия кода.
const SAFETY_Q = [
  ...
  return (
    <div className="shell">
      ...
      <main className="main">
        ...
        <Sec icon="ti-rocket" title="Позиционирование — почему не ChatGPT" sub="Мы не конкурируем с ChatGPT «в лоб». Внутри тот же класс AI; мы — продукт для задачи, которую он структурно не делает.">
        ...
        <Sec icon="ti-lock-check" title="Может ли кто-то удалить или испортить данные?" sub="Короткий ответ: посторонний — нет. Открытие кода этого не изменило. Почему:">
        ...
        <Sec icon="ti-bell" title="Пуши и напоминания" sub="Принцип: мягко помогать вернуться, но не спамить. Язык — из Telegram (RU·EN·UK·FR). Шлёт Vercel Cron раз в день вечером (защита CRON_SECRET); мгновенные — сам бот при записи.">
