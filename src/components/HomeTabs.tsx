...
const GEAR_LABEL: Record<string, string> = { ru: "Настроить", en: "Customize", uk: "Налаштувати", fr: "Personnaliser", es: "Personalizar" };
const PROFILE_LABEL: Record<string, string> = { ru: "Открыть профиль", en: "Open profile", uk: "Відкрити профіль", fr: "Ouvrir le profil", es: "Abrir perfil" };
const ACQ: Record<string, { startT: string; startS: string; startCta: string; contT: string; contCta: string }> = {
  ...
  es: { startT: "Vamos a conocernos", startS: "Responde un par de preguntas cercanas en el bot — y tu primera entrada se escribirá sola.", startCta: "Empezar", contT: "Conociéndonos", contCta: "Continuar" },
};
...
const DAYPART_LINE: Record<string, { morning: string; day: string; evening: string; night: string }> = {
  ...
  es: { morning: "Un buen comienzo. Haz que el día cuente — para ti y para los demás.", day: "Mantén el foco y no olvides a los tuyos.", evening: "Cierra el día: guarda los momentos, nota lo bueno, da las gracias.", night: "Hora tardía. Si hay algo que guardar del día — aquí estoy." },
};
...
const HS: Record<string, any> = {
  ...
  es: { tabs: ["Hoy", "Camino", "Legado"], focus: "Foco del día", tasks: "Tareas principales", gratitude: "Gratitud", memory: "Recuerdo", ask: "Pregúntale a tu vida", askSub: "La IA responde con todas tus entradas", balance: "Balance de vida", recent: "Entradas de hoy", noTasks: "No hay tareas pendientes 👌", noEntries: "Todavía no hay entradas hoy — escribe unas líneas arriba.", goals: "Metas", projects: "Proyectos", lifebook: "Libro de vida", lifebookSub: "La IA convierte los meses en capítulos", insights: "Ideas clave", biographer: "Biógrafo IA", legacyEmpty: "Tu legado crece a medida que escribes.", memYear: (t: string) => `Hace un año en un día como hoy: «${t}»`, memMonth: (t: string) => `Hace un mes en un día como hoy: «${t}»` },
};
...
const HEROLINES: Record<string, string[]> = {
  ...
  es: ["Cada día es una nueva página de tu historia. La escribes con conciencia.", "Hoy se convertirá en parte de tu vida. No dejes que simplemente desaparezca.", "Pequeñas notas hoy son tu memoria para los años que vienen.", "No solo vives el día — lo conservas.", "La vida está hecha de días como este. Y tú no los pierdes."],
};
...
const QUOTES: Record<string, { text: string; author?: string }[]> = {
  ...
  es: [{ text: "Concéntrate en el progreso, no en la perfección." }, { text: "Pequeños pasos cada día lo cambian todo." }, { text: "El mejor momento para empezar es hoy." }, { text: "Eres la suma de tus días." }, { text: "Fíjate en lo bueno — y crecerá." }, { text: "La disciplina es una forma de quererte." }],
};
...
const T0: Record<string, any> = {
  ...
  es: { dayYear: "º día del año", daysLeft: "para que termine el año", days: "d", expDay: "º día del experimento", changed: "Qué cambió desde ayer", stories: "Historias sin terminar", thought: "Pensamiento del día", focus: "Foco del día", up: "Mejor", down: "Peor", flat: "Igual", mood: "Ánimo", sleep: "Sueño", idea: "Nueva idea", appeared: "Apareció", streakWord: "Racha", daysInRow: "días seguidos", exp: "Experimento", expGo: "En curso", yearAgo: "Hace un año en un día como hoy", monthAgo: "Hace un mes en un día como hoy", noStories: "Aquí aparecerán proyectos, metas e ideas a las que vale la pena volver.", notes: "notas", totalDays: "días con entradas", keepChain: "Escribe hoy para no romper tu racha 🔥", chainKept: "Hoy quedó registrado — la racha continúa ✓", startChain: "Empieza tu racha — escribe hoy", traceToday: "Mi huella hoy", traceEmpty: "¿Hiciste algo bueno hoy? Cuéntalo en una entrada — hasta un detalle deja huella.", promised: "Prometiste", traceWeekT: "Tu huella esta semana", deedsWord: "buenas acciones", promisesWord: "promesas cumplidas", gratWord: "gratitudes" },
};
