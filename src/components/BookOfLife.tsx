...
const STR: Record<string, any> = {
  ru: {
    ...
    recipients: { self: "Себе", parents: "Родителям", children: "Детям", partner: "Партнёру", family: "Семье", public: "Публике" },
    ...
    dataLabels: { peopleYear: "Люди этого периода", placesYear: "Места", projects: "Проекты и дела", deeds: "Добрых дел", promises: "Обещаний выполнено", gratitude: "Благодарностей", mood: "Настроение", energy: "Энергия", health: "Здоровье", avg: "в среднем", highlights: "Яркие моменты", curate: "Править", addName: "Добавить", namePh: "Имя…", doneEdit: "Готово", resetAuto: "Вернуть авто", editedTag: "правлено вручную", merge: "Объединить", mergeInto: "Оставить имя", selHint: "Выбери 2+, чтобы объединить", keepName: "Оставить это имя" },
  },
  en: {
    ...
    recipients: { self: "Myself", parents: "Parents", children: "Children", partner: "Partner", family: "Family", public: "The public" },
...
function NameList({ items, label, icon, color, custom, onSave, onReset, s }: any) {
  ...
  const start = () => { idRef.current = 0; setDraft((items || []).map((it: any) => ({ id: nid(), name: it.name, count: it.count || 0 }))); setNewName(""); setSel(new Set()); setPrimaryId(null); setEditing(true); };
  ...
  const toggleSel = (id: number) => setSel((c) => {
    const n = new Set(c);
    if (n.has(id)) { n.delete(id); setPrimaryId((p) => p === id ? null : p); }
    else { n.add(id); setPrimaryId((p) => p == null ? id : p); }
    return n;
  });
  const removeRow = (id: number) => { setDraft((c) => c.filter((r) => r.id !== id)); setSel((c) => { const n = new Set(c); n.delete(id); return n; }); setPrimaryId((p) => p === id ? null : p); };
  // Объединить выбранные в одну сущность: имя берём у отмеченной ★ (или первой выбранной), упоминания суммируем.
  const mergeSel = () => {
    if (sel.size < 2) return;
    setDraft((d) => {
      const selRows = d.filter((r) => sel.has(r.id));
      const total = selRows.reduce((a, r) => a + (r.count || 0), 0);
      const target = selRows.find((r) => r.id === primaryId) || selRows[0];
      return d.filter((r) => r.id === target.id || !sel.has(r.id)).map((r) => r.id === target.id ? { ...r, count: total } : r);
    });
    setSel(new Set());
    setPrimaryId(null);
  };
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 10 }}>
          {draft.map((d) => {
            ...
            const isPrimary = on && primaryId === d.id;
            return (
              <div key={d.id} style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--surface-2)", borderRadius: 10, padding: "5px 6px 5px 7px", border: on ? "1.5px solid var(--accent)" : "1.5px solid transparent" }}>
                ...
                {on && (
                  <button onClick={() => setPrimaryId(d.id)} title={L.keepName}
                    style={{ flexShrink: 0, width: 22, height: 22, display: "inline-flex", alignItems: "center", justifyContent: "center", borderRadius: 6, cursor: "pointer", border: "none", background: "none", color: isPrimary ? "#f59e0b" : "var(--text-3)", padding: 0 }}>
                    <i className={`ti ${isPrimary ? "ti-star-filled" : "ti-star"}`} style={{ fontSize: 15 }} />
                  </button>
                )}
                ...
                <input value={d.name} onChange={(e) => setDraft((c) => c.map((x) => x.id === d.id ? { ...x, name: e.target.value } : x))}
                  style={{ flex: 1, minWidth: 0, background: "transparent", border: "none", outline: "none", color: "var(--text)", fontSize: 13.5, fontWeight: isPrimary ? 600 : 400, padding: "2px 0" }} />
        ...
        {sel.size >= 2 && (() => {
          const selRows = draft.filter((r) => sel.has(r.id));
          const keep = (selRows.find((r) => r.id === primaryId) || selRows[0])?.name?.trim();
          return (
            <button onClick={mergeSel} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: 9, border: "none", background: "var(--accent)", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", marginBottom: 12 }}>
              <i className="ti ti-arrows-join" style={{ fontSize: 15 }} />{L.merge} · {sel.size}{keep ? ` → «${keep}»` : ""}
            </button>
          );
        })()}
