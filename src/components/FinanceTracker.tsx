...
export default function FinanceTracker({ data, locale }: { data: Data; locale: string }) {
  ...
  async function importMono() {
    ...
    try {
      ...
      if (j?.ok) { setMonoMsg(s.monoImported(j.inserted || 0) + (j.rateLimited ? " " + s.monoImportPartial : "")); router.refresh(); }
