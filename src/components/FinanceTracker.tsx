...
type Data = {
  month: string; currency: string; rates: Record<string, number>; currenciesUsed: string[]; needsRates: boolean; autoRates?: Record<string, number>;
...
const STR: Record<string, any> = {
  ...
  es: { balance: "Balance del mes", income: "Ingresos", expense: "Gastos", add: "Añadir", addIncome: "Ingreso", addExpense: "Gasto", amount: "Importe", category: "Categoría", date: "Fecha", note: "Nota (opcional)", save: "Guardar", cancel: "Cancelar", byCategory: "Gastos por categoría", operations: "Movimientos", empty: "Este mes no hay movimientos. Toca «Añadir» para registrar un ingreso o gasto.", emptyAll: "Aquí vivirán tus ingresos y gastos. Añade tu primer movimiento y tendrás una imagen clara de tu dinero.", delConfirm: "¿Eliminar este movimiento?", noCat: "Sin categoría", today: "Hoy", yesterday: "Ayer", currency: "Moneda", pickPeriod: "Elegir mes y año", earliest: "A los primeros", thisMonth: "Mes actual", calendar: "Calendario del mes", weekdays: ["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"], ops: "mov.", dayBalance: "Balance del día", allDays: "Mostrar todos los días", selectedDayLabel: "Movimientos del día", opsMore: (n: number) => `Mostrar ${n} más`, opsCollapse: "Contraer", opsShowAll: "Mostrar todo", opsMovedNote: "Los movimientos están desplegados abajo, bajo las categorías.", subcategoryPh: "Subcategoría (p. ej. Deporte) — opcional", subSuggest: ["Deporte", "Educación", "Ropa", "Salud", "Comida", "Ocio", "Regalos", "Transporte"], exportCsv: "Exportar a CSV", trendTitle: "Tendencia mensual", trendShow: "Mostrar gráfico", trendLoading: "Cargando…", trendEmpty: "Todavía no hay suficientes datos para un gráfico.", adviceTitle: "Asesor financiero con IA", adviceGet: "Obtener análisis y consejos", adviceThinking: "Analizando tus finanzas…", adviceAgain: "Actualizar", adviceErr: "No se pudo generar el análisis. Inténtalo más tarde.", recurTitle: "Pagos recurrentes", recurHint: "Alquiler, suscripciones, préstamos. El bot te avisará el día del pago — tú decides si registrarlo.", recurAdd: "Añadir pago", recurEmpty: "Todavía no hay pagos recurrentes.", recurDay: "Día del mes", recurEvery: (d: number) => `el día ${d} de cada mes`, recurDelConfirm: "¿Eliminar este pago recurrente?", monoTitle: "Banco Monobank", monoHint: "Conecta tu tarjeta — las nuevas compras e ingresos entrarán automáticamente en «Dinero».", monoTokenPh: "Pega tu token de Monobank", monoGetToken: "Obtener un token en api.monobank.ua", monoConnect: "Conectar", monoConnected: (n: string | null) => `✅ Conectado${n ? `: ${n}` : ""}. Los nuevos movimientos aparecerán automáticamente.`, monoDisconnect: "Desconectar", monoDisconnectConfirm: "¿Desconectar Monobank? Los movimientos ya añadidos se mantendrán.", monoErr: "No se pudo conectar. Revisa el token e inténtalo de nuevo.", monoBadToken: "Token inválido. Cópialo de nuevo desde api.monobank.ua.", monoRate: "Monobank pide esperar un minuto — inténtalo de nuevo en breve.", monoWebhookWarn: "Token guardado, pero el webhook aún no se activó. Los movimientos se sincronizarán más tarde — intenta reconectar en un minuto.", monoImportBtn: "Importar últimos 30 días", monoImported: (n: number) => `Se importaron ${n} movimientos.`, monoImportPartial: "Si tienes varias cuentas, espera un minuto y pulsa «Importar» de nuevo para cargar el resto.", monoFixed: (n: number) => `Moneda corregida en ${n}.`, goalsTitle: "Metas de ahorro", goalsHint: "Ahorra para una meta y sigue tu progreso. Añade dinero manualmente o después de un ingreso.", goalAdd: "Nueva meta", goalsEmpty: "Todavía no hay metas. Añade la primera — p. ej. «Vacaciones, 2000 €».", goalTitlePh: "Título (p. ej. Coche nuevo)", goalTargetPh: "Importe objetivo", goalLeft: (rest: string) => `faltan ${rest}`, goalDone: "¡Meta alcanzada! 🎉", goalContribute: "Añadir dinero", goalContributePrompt: (cur: string) => `¿Cuánto añadir (${cur})? Puede ser negativo para restar:`, goalDelConfirm: "¿Eliminar esta meta?",
    budgets: "Presupuestos por categoría", limit: "Límite", setLimit: "Fijar un límite", editLimit: "Editar límite", removeLimit: "Quitar límite", ofLimit: "de", over: "excedido en", leftWord: "restante", addBudget: "Añadir un límite", budgetTotalT: "Presupuesto del mes", spent: "gastado", budgetAllHint: "Un límite global para todos los gastos del mes.", budgetCatsHint: "Se calcula con las categorías con límite. Puedes fijar un límite global.", budgetSetAll: "Fijar límite mensual global", budgetAllPh: "Límite del mes",
    settings: "Ajustes y monedas", baseCurrency: "Moneda principal", ratesT: "Tasas frente a la moneda principal", rateLine: (c: string, b: string) => `1 ${c} =`, rateAuto: "auto · NBU este mes", needsRatesWarn: "Los totales son aproximados: indica las tasas de cambio en ajustes para contarlo todo en una sola moneda.", ratesHint: "Cómo funciona: las tasas se fijan solas — la tasa oficial del NBU del mes de cada operación (los meses antiguos usan sus propias tasas). El valor gris del campo es la tasa que la automática aplica ahora. Deja los campos vacíos; un valor manual solo actúa de respaldo si el NBU no está disponible.", histNote: "Los importes en distintas monedas se convierten a la moneda principal según la tasa oficial del NBU del mes de cada operación — las operaciones de 2020 y 2023 se calculan con sus propias tasas, no con la de hoy.",
    importTitle: "Migrar desde MoneyOK", importBtn: "Elegir archivo MoneyOK.csv", importing: "Importando movimientos…", importHint: "En MoneyOK: Menú → «Exportar a CSV» → envíate el archivo y súbelo aquí. Se migrarán todos los ingresos y gastos. Volver a subir el mismo archivo no crea duplicados.", importDone: (n: number, dup: number, skip: number) => `Se importaron ${n} movimientos${dup ? `, ${dup} duplicados omitidos` : ""}${skip ? `, ${skip} transferencias/saldos omitidos` : ""}.`, importEmpty: "No se reconoció ningún movimiento en el archivo. ¿Seguro que es una exportación CSV de MoneyOK?", importErr: "No se pudo subir el archivo. Inténtalo de nuevo.", importUntagged: " Nota: no se pudieron etiquetar los movimientos (base de datos antigua), la deshacer con un clic no estará disponible — actualiza el esquema supabase/finance.sql.", undoBtn: "Deshacer importación de MoneyOK", undoConfirm: "¿Eliminar todos los movimientos importados de MoneyOK? Los añadidos manualmente se mantendrán.", undoDone: (n: number) => `Deshecho: se eliminaron ${n} movimientos.`, undoNone: "No se encontraron movimientos importados — nada que eliminar.", undoErr: "No se pudo deshacer. Inténtalo de nuevo." },
};

// Категории расходов и доходов: ключ, эмодзи, цвет, названия на 4 языках.
const EXPENSE_CATS = [
  { key: "food", icon: "🛒", color: "#22c55e", l: { ru: "Продукты", en: "Groceries", uk: "Продукти", fr: "Courses", es: "Supermercado" } },
  { key: "cafe", icon: "☕", color: "#f97316", l: { ru: "Кафе", en: "Eating out", uk: "Кафе", fr: "Restos", es: "Restaurantes" } },
  { key: "transport", icon: "🚗", color: "#3b82f6", l: { ru: "Транспорт", en: "Transport", uk: "Транспорт", fr: "Transport", es: "Transporte" } },
  { key: "housing", icon: "🏠", color: "#8b5cf6", l: { ru: "Жильё", en: "Housing", uk: "Житло", fr: "Logement", es: "Vivienda" } },
  { key: "bills", icon: "🧾", color: "#0ea5e9", l: { ru: "Счета и связь", en: "Bills", uk: "Рахунки", fr: "Factures", es: "Facturas" } },
  { key: "shopping", icon: "🛍️", color: "#ec4899", l: { ru: "Покупки", en: "Shopping", uk: "Покупки", fr: "Achats", es: "Compras" } },
  { key: "health", icon: "💊", color: "#ef4444", l: { ru: "Здоровье", en: "Health", uk: "Здоров'я", fr: "Santé", es: "Salud" } },
  { key: "fun", icon: "🎬", color: "#a855f7", l: { ru: "Развлечения", en: "Fun", uk: "Розваги", fr: "Loisirs", es: "Ocio" } },
  { key: "education", icon: "📚", color: "#14b8a6", l: { ru: "Образование", en: "Education", uk: "Освіта", fr: "Éducation", es: "Educación" } },
  { key: "travel", icon: "✈️", color: "#06b6d4", l: { ru: "Путешествия", en: "Travel", uk: "Подорожі", fr: "Voyages", es: "Viajes" } },
  { key: "gifts", icon: "🎁", color: "#f43f5e", l: { ru: "Подарки", en: "Gifts", uk: "Подарунки", fr: "Cadeaux", es: "Regalos" } },
  { key: "other", icon: "💸", color: "#64748b", l: { ru: "Другое", en: "Other", uk: "Інше", fr: "Autre", es: "Otros" } },
];
const INCOME_CATS = [
  { key: "salary", icon: "💼", color: "#16a34a", l: { ru: "Зарплата", en: "Salary", uk: "Зарплата", fr: "Salaire", es: "Salario" } },
  { key: "freelance", icon: "💻", color: "#0ea5e9", l: { ru: "Подработка", en: "Freelance", uk: "Підробіток", fr: "Freelance", es: "Freelance" } },
  { key: "business", icon: "📈", color: "#8b5cf6", l: { ru: "Бизнес", en: "Business", uk: "Бізнес", fr: "Business", es: "Negocio" } },
  { key: "gift", icon: "🎁", color: "#f43f5e", l: { ru: "Подарок", en: "Gift", uk: "Подарунок", fr: "Cadeau", es: "Regalo" } },
  { key: "investment", icon: "💰", color: "#eab308", l: { ru: "Инвестиции", en: "Investments", uk: "Інвестиції", fr: "Investissements", es: "Inversiones" } },
  { key: "other", icon: "➕", color: "#64748b", l: { ru: "Другое", en: "Other", uk: "Інше", fr: "Autre", es: "Otros" } },
];
...
function intlOf(l: string) { return l === "en" ? "en-GB" : l === "fr" ? "fr-FR" : l === "es" ? "es-ES" : l === "uk" ? "uk-UA" : "ru-RU"; }
...
export default function FinanceTracker({ data, locale }: { data: Data; locale: string }) {
  ...
  // Операции: компактный блок прокручивается внутри себя; «Показать всё»
  // разворачивает их широким блоком под категориями.
  const [opsWide, setOpsWide] = useState(false);
  ...
  // Общий лимит месяца (finance_budget, category="__all__").
  const [allBudgetEdit, setAllBudgetEdit] = useState(false);
  const [allBudgetVal, setAllBudgetVal] = useState("");
  async function saveAllBudget() {
    const v = parseFloat(allBudgetVal.replace(",", "."));
    if (!isFinite(v) || v <= 0) return;
    setBusy(true);
    await fetch("/api/finance-budget", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ category: "__all__", amount: v }) }).catch(() => {});
    setBusy(false); setAllBudgetEdit(false); router.refresh();
  }
  async function removeAllBudget() {
    setBusy(true);
    await fetch("/api/finance-budget", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ category: "__all__" }) }).catch(() => {});
    setBusy(false); setAllBudgetEdit(false); router.refresh();
  }
  ...
  // Список операций по дням — один на два места: компактный блок в сетке
  // (прокручивается внутри себя) и широкий «Показать всё» под категориями.
  function renderOpsDays() {
    return visDays.map(({ day: d, items }) => (
  ...
  return (
    <div className="fin-wrap" style={{ position: "relative", paddingBottom: 72 }}>
      ...
      {(budgetTotal || data.hasAny) && (
        <div className="card" style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: budgetTotal ? 8 : 0, gap: 8, flexWrap: "wrap" }}>
            <span style={{ fontSize: 13, color: "var(--text-2)", display: "inline-flex", alignItems: "center", gap: 6 }}>
              <i className="ti ti-target" style={{ fontSize: 15, color: "var(--accent)" }} />{s.budgetTotalT}
              <button onClick={() => { setAllBudgetEdit((v) => !v); setAllBudgetVal(budgetTotal?.mode === "all" ? String(budgetTotal.limit) : ""); }}
                title={budgetTotal?.mode === "all" ? s.editLimit : s.budgetSetAll}
                style={{ background: "none", border: "none", cursor: "pointer", color: budgetTotal?.mode === "all" ? "var(--accent)" : "var(--text-3)", padding: 2 }}>
                <i className={`ti ${budgetTotal?.mode === "all" ? "ti-edit" : "ti-target-arrow"}`} style={{ fontSize: 14 }} />
              </button>
            </span>
            {budgetTotal ? (
              ...
              <button onClick={() => { setAllBudgetEdit(true); setAllBudgetVal(""); }} style={{ ...btnG, padding: "5px 11px", fontSize: 12 }}>{s.budgetSetAll}</button>
            )}
          </div>
          {allBudgetEdit && (
            <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", margin: "4px 0 10px" }}>
              <input autoFocus type="number" inputMode="decimal" placeholder={`${s.budgetAllPh}, ${symOf(base)}`} value={allBudgetVal} onChange={(e) => setAllBudgetVal(e.target.value)} style={{ ...input, width: 150, padding: "6px 9px" }} />
              <button disabled={busy} onClick={saveAllBudget} style={{ ...btnP, padding: "6px 12px" }}>{s.save}</button>
              {budgetTotal?.mode === "all" && <button disabled={busy} onClick={removeAllBudget} style={{ ...btnG, padding: "6px 12px", color: "#ef4444" }}>{s.removeLimit}</button>}
              <button disabled={busy} onClick={() => setAllBudgetEdit(false)} style={{ ...btnG, padding: "6px 12px" }}>{s.cancel}</button>
            </div>
          )}
          {budgetTotal && (
            <>
              ...
              <div style={{ fontSize: 12, color: budgetTotal.over ? "#ef4444" : "var(--text-3)", marginTop: 6 }}>
                ...
                <span style={{ color: "var(--text-3)" }}> · {budgetTotal.mode === "all" ? s.budgetAllHint : s.budgetCatsHint}</span>
              </div>
            </>
          )}
        </div>
      ...
      {/* Все операции широким блоком — по кнопке «Показать всё» из сетки */}
      {opsWide && txs.length > 0 && (
        <div className="card" id="fin-ops-wide" style={{ marginBottom: 14, scrollMarginTop: 14 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10, gap: 8 }}>
            <span style={{ fontSize: 13, color: "var(--text-2)", display: "inline-flex", alignItems: "center", gap: 6 }}>
              <i className="ti ti-list" style={{ fontSize: 15, color: "var(--accent)" }} />{s.operations}
              <span style={{ color: "var(--text-3)", fontSize: 12 }}>· {opsCount}</span>
            </span>
            <button onClick={() => { setOpsWide(false); setTimeout(() => document.getElementById("fin-ops")?.scrollIntoView({ behavior: "smooth", block: "start" }), 120); }}
              style={{ ...btnG, padding: "6px 12px", fontSize: 12.5, display: "inline-flex", alignItems: "center", gap: 6 }}>
