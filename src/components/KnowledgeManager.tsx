...
export default function KnowledgeManager({ initial, locale, emptyHint }: { initial: SavedItem[]; locale: string; emptyHint?: string }) {
  ...
  const chip = (active: boolean): React.CSSProperties => ({ fontSize: 12, padding: "4px 11px", borderRadius: 999, cursor: "pointer", border: "1px solid " + (active ? "#6d5efc" : "var(--border)"), background: active ? "rgba(109,94,252,.12)" : "var(--card)", color: active ? "#6d5efc" : "var(--text-2,var(--muted))", whiteSpace: "nowrap" });
    ...
    <div>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input value={addUrl} onChange={(e) => { setAddUrl(e.target.value); setAddErr(""); }} onKeyDown={(e) => { if (e.key === "Enter" && !addBusy) add(); }} placeholder={s.addPh} disabled={addBusy} style={{ flex: 1, minWidth: 220, padding: "11px 13px", borderRadius: 11, border: "1px solid var(--border)", background: "var(--card)", color: "var(--text)", fontSize: 13.5 }} />
      ...
      <div style={{ marginBottom: 14 }}>
        {!askOpen ? (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button onClick={translateBase} disabled={trBusy} title={s.translate} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "8px 13px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--card)", color: "var(--text-2,var(--muted))", fontSize: 13, fontWeight: 600, cursor: trBusy ? "default" : "pointer" }}>
          ...
          <div style={{ border: "1px solid var(--border)", borderRadius: 12, background: "var(--card)", padding: 12 }}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <input value={askQ} onChange={(e) => setAskQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !askBusy) ask(); }} placeholder={s.askPh} autoFocus disabled={askBusy} style={{ flex: 1, minWidth: 220, padding: "10px 12px", borderRadius: 10, border: "1px solid var(--border)", background: "var(--surface,var(--card))", color: "var(--text)", fontSize: 13.5 }} />
              ...
              <button onClick={() => { setAskOpen(false); setAskA(""); }} aria-label="close" style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 18, padding: "0 6px" }}><i className="ti ti-x" /></button>
            </div>
      ...
      <div style={{ marginBottom: 18 }}>
        <div style={{ position: "relative", marginBottom: 9 }}>
          <i className="ti ti-search" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--muted)", fontSize: 15 }} />
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={s.searchPh} style={{ width: "100%", boxSizing: "border-box", padding: "10px 12px 10px 34px", borderRadius: 11, border: "1px solid var(--border)", background: "var(--card)", color: "var(--text)", fontSize: 13.5 }} />
        </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ padding: 26, border: "1px solid var(--border)", borderRadius: 16, background: "var(--card)", color: "var(--muted)", textAlign: "center", lineHeight: 1.6 }}>{items.length === 0 ? (emptyHint || s.empty) : s.empty}</div>
      ) : (
        groups.map(([topic, list]) => (
          <section key={topic} style={{ marginBottom: 26 }}>
            <h2 style={{ fontSize: 14, fontWeight: 700, margin: "0 0 11px", display: "flex", alignItems: "center", gap: 7 }}>
              ...
              {topic !== "—" ? <button onClick={() => renameFolder(topic)} aria-label="rename" title={s.edit} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 13, padding: 2 }}><i className="ti ti-pencil" /></button> : null}
            </h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 12 }}>
              {list.map((it) => (
                <article
                  ...
                  style={{ border: "1px solid var(--border)", borderRadius: 14, background: "var(--card)", overflow: "hidden", cursor: "pointer", display: "flex", flexDirection: "column", opacity: dragId === it.id ? 0.5 : 1, position: "relative" }}
                >
                  ...
                  <div style={{ padding: "10px 11px 12px", display: "flex", flexDirection: "column", gap: 5 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 1 }}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); setMenuId(menuId === it.id ? null : it.id); }} aria-label="menu" style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 16, padding: 2, flexShrink: 0 }}><i className="ti ti-dots" /></button>
                    </div>
                    {menuId === it.id ? (
                      <div onClick={(e) => e.stopPropagation()} style={{ position: "absolute", right: 10, bottom: 10, zIndex: 5, background: "var(--card)", border: "1px solid var(--border)", borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,.18)", padding: 6, minWidth: 150 }}>
                        <button onClick={() => { setOpen(it); startEdit(it); setMenuId(null); }} style={menuItem}><i className="ti ti-pencil" /> {s.edit}</button>
                        <div style={{ padding: "4px 8px 2px", fontSize: 10.5, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".04em" }}>{s.folder}</div>
      ...
      {open ? (
        <div onClick={() => { setOpen(null); setEditing(false); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 1000, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: 20, overflowY: "auto" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--card)", borderRadius: 18, maxWidth: 580, width: "100%", margin: "auto", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,.3)" }}>
            <div style={{ position: "relative" }}>
              <Thumb it={open} h={230} />
              ...
              {editing ? (
                <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 2 }}>
                    <button onClick={() => setEditing(false)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: 13.5, padding: "8px 10px" }}>{s.cancel}</button>
                ...
                <>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 18, flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => startEdit(open)} style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "none", border: "none", color: "var(--text-2,var(--muted))", cursor: "pointer", fontSize: 13 }}><i className="ti ti-pencil" /> {s.edit}</button>
...
const lbl: React.CSSProperties = { display: "grid", gap: 5, fontSize: 11.5, color: "var(--muted)", fontWeight: 600 };
const inp: React.CSSProperties = { padding: "9px 11px", borderRadius: 9, border: "1px solid var(--border)", background: "var(--surface,var(--card))", color: "var(--text)", fontSize: 13.5, width: "100%", boxSizing: "border-box" };
