...
export default async function ProfileBody({ user, locale }: { user: CurrentUser; locale: string }) {
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      {/* Мои устройства: часы и кнопка-брелок — запись без телефона в руках */}
      <Link href="/profile/devices" className="card" style={{ display: "flex", alignItems: "center", gap: 13, marginBottom: 16 }}>
        <i className="ti ti-device-watch" style={{ fontSize: 22, color: "var(--accent)", flexShrink: 0 }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 14, fontWeight: 600 }}>{s.devT}</div>
          <div style={{ fontSize: 12.5, color: "var(--text-2)", lineHeight: 1.45 }}>{s.devS}</div>
        </div>
        <span style={{ fontSize: 12.5, color: "var(--accent)", fontWeight: 500, whiteSpace: "nowrap" }}>{s.refBtn} →</span>
      </Link>
