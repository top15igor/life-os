...
export default async function ProfileBody({ user, locale, notice }: { user: CurrentUser; locale: string; notice?: "linked" | "emailtaken" }) {
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      ...
      {/* PIN: прячем для тех, у кого есть почта/Google (защита уже есть). Если PIN уже стоял — показываем, чтобы можно было снять. */}
      {(!email || hasPin) && (
        <>
          {email && hasPin && (
            <div style={{ fontSize: 12, color: "var(--text-3)", margin: "0 0 8px", lineHeight: 1.45 }}>{s.pinNote}</div>
          )}
