      ...
      {items.map((p) => {
        ...
        return (
          <div key={p.id} className="card" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, opacity: done ? 0.55 : 1 }}>
            ...
            {done ? (
              <span style={{ fontSize: 12, color: "var(--positive)", whiteSpace: "nowrap", flexShrink: 0 }}>{s.kept}</span>
            ) : (
              <button onClick={() => markDone(p.id)} style={{ fontSize: 12.5, padding: "6px 12px", borderRadius: 9, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--accent)", cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0 }}>
                {s.done}
              </button>
            )}
          </div>
