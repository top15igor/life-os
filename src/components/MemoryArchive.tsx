...
type Memory = { id: string; category: string; title: string; summary: string; fields: { label: string; value: string }[]; folder?: string | null; remind_at?: string | null; remind_off?: boolean; mem_date: string | null; image_url: string | null; status: string; note?: string | null; file_url?: string | null; file_name?: string | null; mime_type?: string | null; created_at: string };
...
export default function MemoryArchive({ initial, locale, catLabels: catLabelsInit, customCats: customCatsInit }: { initial: Memory[]; locale: string; catLabels?: Record<string, string>; customCats?: { key: string; label: string; icon: string; c: string; bg: string }[] }) {
  ...
  async function refreshDates() {
    ...
    try {
      const r = await fetch("/api/admin/memory-reanalyze", { method: "POST" }).then((x) => x.json());
      if (r?.ok) { setScanMsg(r.updated ? s.scanDone + ` (${r.updated})` : s.scanNone); if (r.updated) setTimeout(() => window.location.reload(), 1000); }
    } catch {}
    setScanning(false);
  }
  ...
  function openMenu(e: any, id: string, type: "move" | "cat" | "send" | "remind") {
    e.stopPropagation();
  ...
  const [catLabels, setCatLabels] = useState<Record<string, string>>(catLabelsInit || {});
  const [customCats, setCustomCats] = useState(customCatsInit || []);
  // Все категории = 8 базовых + пользовательские. Используем везде для чипов и меню.
  const allCats = [...CATS, ...customCats.map((c) => ({ key: c.key, icon: c.icon, c: c.c, bg: c.bg }))];
  const catMetaOf = (k: string) => allCats.find((x) => x.key === k) || CATS[7];
  const customLabel = (key: string) => customCats.find((c) => c.key === key)?.label;
  const catName = (key: string) => catLabels[key] || s.catNames[key] || customLabel(key) || key;
  async function addCategory(assignId?: string) {
    const label = (prompt(s.newCat) || "").trim();
    if (!label) return;
    try {
      const r = await fetch("/api/memory", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "addCat", label }) }).then((x) => x.json());
      if (r?.ok && r.cat) {
        setCustomCats((p) => [...p, r.cat]);
        if (assignId) setCategory(assignId, r.cat.key);
      }
    } catch {}
  }
  async function renameCategory(key: string) {
    const cur = catName(key);
    const to = (prompt(s.renameCat, cur) || "").trim();
    if (!to || to === cur) return;
    setCatLabels((p) => ({ ...p, [key]: to }));
    try { await fetch("/api/memory", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "catLabel", category: key, label: to }) }); } catch {}
  }

  const folderOf = (m: Memory) => (m.folder && m.folder.trim()) || deriveFolder(m.category, m.title, m.fields) || null;
  const isRealFolder = (arr: Memory[]) => arr.length >= 2 || arr.some((m) => !!(m.folder && m.folder.trim()));
  const effExpiry = (m: Memory): string | null => {
    if (m.remind_off) return null;
    if (m.remind_at && /^\d{4}-\d{2}-\d{2}$/.test(m.remind_at)) return m.remind_at;
    return documentExpiry(m.category, m.fields)?.date || null;
  };
  const isDocLike = (m: Memory) => ["document", "thing", "info"].includes(m.category);
  async function setRemind(id: string, patch: { remindAt?: string | null; off?: boolean }) {
    setItems((p) => p.map((m) => {
      if (m.id !== id) return m;
      const n = { ...m };
      if (patch.off !== undefined) n.remind_off = patch.off;
      if (patch.remindAt !== undefined) { n.remind_at = patch.remindAt; if (patch.remindAt) n.remind_off = false; }
      return n;
    }));
    setMenu(null);
    try { await fetch("/api/memory", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "remind", id, ...patch }) }); } catch {}
  }
  ...
  const expiring = items
    .map((m) => { const d = effExpiry(m); return d ? { m, date: d, days: daysLeft(d, todayISO) } : null; })
    .filter((x): x is { m: Memory; date: string; days: number } => !!x && x.days <= 365)
    .sort((a, b) => a.days - b.days)
    .slice(0, 6);

  const used = allCats.filter((c) => items.some((m) => m.category === c.key));

  const Card = (m: Memory) => {
    const cm = catMetaOf(m.category);
        ...
        <div style={{ padding: "12px 13px 13px" }}>
          ...
          {!manage && (
            <div style={{ display: "flex", alignItems: "center", gap: 2, marginTop: 10, borderTop: "1px solid var(--border)", paddingTop: 8 }}>
              ...
              {(isDocLike(m) || effExpiry(m)) && <button onClick={(e) => openMenu(e, m.id, "remind")} title={s.remindLbl} style={{ flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", padding: "6px 4px", borderRadius: 8, border: "none", background: menu && menu.id === m.id && menu.type === "remind" ? "var(--surface-2)" : "none", color: effExpiry(m) ? "#d97706" : "var(--text-2)", cursor: "pointer" }}><i className="ti ti-clock" style={{ fontSize: 15 }} /></button>}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 9, flexWrap: "wrap" }}>
            <button onClick={() => setCatId(catId === m.id ? null : m.id)} style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, fontWeight: 500, color: cm.c, background: cm.bg, padding: "2px 8px", borderRadius: 999, border: "none", cursor: "pointer" }}><i className={`ti ${cm.icon}`} style={{ fontSize: 12 }} />{catName(m.category)}<i className="ti ti-chevron-down" style={{ fontSize: 11 }} /></button>
          </div>

          {catId === m.id && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, margin: "8px 0 3px" }}>
              {allCats.map((c) => (
                <button key={c.key} onClick={() => setCategory(m.id, c.key)} style={{ fontSize: 11.5, padding: "4px 9px", borderRadius: 999, cursor: "pointer", border: "1px solid " + (m.category === c.key ? c.c : "var(--border)"), background: m.category === c.key ? c.bg : "var(--surface)", color: m.category === c.key ? c.c : "var(--text-2)" }}>{catName(c.key)}</button>
              ))}
              <button onClick={() => addCategory(m.id)} style={{ fontSize: 11.5, padding: "4px 9px", borderRadius: 999, cursor: "pointer", border: "1px dashed var(--border)", background: "var(--surface)", color: "var(--accent)" }}>+ {s.newCat.replace(/[:：].*$/, "")}</button>
            </div>
          )}
  ...
  function folderize(arr: Memory[]) {
    ...
    const folderKeys = order.filter((k) => k !== "__none__" && isRealFolder(groups.get(k)!));
        ...
        return (
          <>
            ...
            <div style={{ position: "fixed", left, ...pos, zIndex: 61, minWidth: 210, maxHeight: menu.maxH, overflowY: "auto", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, boxShadow: "0 10px 30px rgba(0,0,0,.18)", padding: 6 }}>
              ...
              {menu.type === "remind" && <>
                <div style={{ fontSize: 12, color: "var(--text-3)", padding: "6px 11px 8px" }}>{effExpiry(m) ? `${s.remindLbl}: ${dateStr(effExpiry(m)!)}` : (m.remind_off ? s.remindOff : s.noExp)}</div>
                {item(s.remindHalf, "ti-clock", () => setRemind(m.id, { remindAt: plusMonths(6) }))}
                {item(s.remind1y, "ti-clock", () => setRemind(m.id, { remindAt: plusMonths(12) }))}
                {item(s.remind2y, "ti-clock", () => setRemind(m.id, { remindAt: plusMonths(24) }))}
                {item(s.remindDate, "ti-calendar", () => { const d = (prompt(s.remindAsk, effExpiry(m) || "") || "").trim(); if (/^\d{4}-\d{2}-\d{2}$/.test(d)) setRemind(m.id, { remindAt: d }); })}
                {m.remind_off
                  ? item(s.remindOn, "ti-bell", () => setRemind(m.id, { off: false }), "var(--accent)")
                  : item(s.remindOff, "ti-bell-off", () => setRemind(m.id, { off: true }), "var(--text-2)")}
              </>}
              {menu.type === "cat" && <>
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        {items.length > 1 && (
          <div style={{ display: "flex", gap: 16, justifyContent: "center", marginTop: 8 }}>
            ...
            <button onClick={refreshDates} disabled={scanning} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "none", border: "none", color: scanning ? "var(--text-3)" : "var(--accent)", cursor: scanning ? "default" : "pointer", fontSize: 12.5, padding: 0 }}>
              <i className={`ti ${scanning ? "ti-loader-2" : "ti-clock-search"}`} style={{ fontSize: 15 }} />{scanning ? s.scanBusy : scanMsg || s.scan}
            </button>
          </div>
        )}
      </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 15 }}>
            ...
            {used.map((c) => (
              <button key={c.key} onClick={() => { setActiveCat(activeCat === c.key ? "all" : c.key); setPath([]); }} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12.5, fontWeight: 500, padding: "6px 12px", borderRadius: 999, cursor: "pointer", border: "1px solid " + (activeCat === c.key ? c.c : "var(--border)"), background: activeCat === c.key ? c.bg : "var(--surface)", color: activeCat === c.key ? c.c : "var(--text-2)" }}>
                <i className={`ti ${c.icon}`} style={{ fontSize: 14 }} />{catName(c.key)}<span style={{ opacity: 0.75 }}>{items.filter((m) => m.category === c.key).length}</span>
              </button>
            ))}
          </div>
          {activeCat === "all" ? (
              ...
              return (
                <div key={c.key} style={{ marginBottom: 24 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 9, margin: "0 2px 10px" }}>
                    <button onClick={() => { setActiveCat(c.key); setPath([]); }} style={{ display: "flex", alignItems: "center", gap: 9, background: "none", border: "none", padding: 0, cursor: "pointer", color: "var(--text)" }}>
                      ...
                      <span style={{ fontSize: 15.5, fontWeight: 600 }}>{catName(c.key)}</span>
                    ...
                    <button onClick={() => renameCategory(c.key)} title={s.renameCat} style={{ background: "none", border: "none", color: "var(--text-3)", cursor: "pointer", padding: "2px 4px" }}><i className="ti ti-pencil" style={{ fontSize: 14 }} /></button>
                  </div>
                  {(() => {
            ...
            (() => {
              ...
              const directRaw = under.filter((m) => pathOf(m).length === path.length);
              const subOrderAll: string[] = [];
              const subMapAll = new Map<string, Memory[]>();
              for (const m of under) {
                const pp = pathOf(m);
                if (pp.length > path.length) {
                  const seg = pp[path.length];
                  if (!subMapAll.has(seg)) { subMapAll.set(seg, []); subOrderAll.push(seg); }
                  subMapAll.get(seg)!.push(m);
                }
              }
              // Настоящие подпапки (2+ ИЛИ явно созданные); одиночные эвристические —
              // как обычные карточки на этом уровне, чтобы лента и «внутри» совпадали.
              const subOrder = subOrderAll.filter((seg) => isRealFolder(subMapAll.get(seg)!));
              const subMap = subMapAll;
              const looseFromSubs = subOrderAll.filter((seg) => !subOrder.includes(seg)).flatMap((seg) => subMapAll.get(seg)!);
              const direct = [...directRaw, ...looseFromSubs];
                ...
                <div style={{ display: "grid", gap: 18 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", fontSize: 13.5 }}>
                    <button onClick={() => setPath([])} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: path.length ? "var(--accent)" : "var(--text)", fontWeight: 600, padding: 0 }}>
                      <span style={{ width: 24, height: 24, borderRadius: 7, background: cm.bg, display: "flex", alignItems: "center", justifyContent: "center" }}><i className={`ti ${cm.icon}`} style={{ fontSize: 14, color: cm.c }} /></span>{catName(activeCat)}
                    </button>
                    {path.map((seg, i) => (
                    ...
                    {path.length === 0 && <button onClick={() => renameCategory(activeCat)} title={s.renameCat} style={{ background: "none", border: "none", color: "var(--text-3)", cursor: "pointer", padding: "2px 4px" }}><i className="ti ti-pencil" style={{ fontSize: 14 }} /></button>}
                  </div>
