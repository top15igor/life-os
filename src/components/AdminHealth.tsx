...
// статус», а делает настоящую работу: полная проверка шлёт боту живые
// сообщения через настоящий вебхук и судит ответы второй моделью — это
...
export default function AdminHealth() {
  ...
  async function run(kind: "light" | "full" | "diagnose") {
    ...
    try {
      // Полный прогон идёт частями: целиком он не укладывался в отведённое
      // функции время и обрывался без результата. Для глаз это по-прежнему одна
      // кнопка и один общий список. Части три: у роутера появились повторы с
      // паузами при перегрузке, сценарии стали дольше, и половины не влезали.
      const parts = kind === "full" ? [0, 1, 2] : [0];
      const merged: Step[] = [];
      let j: any = null;
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 12.5, color: "var(--text-3)", lineHeight: 1.5, marginTop: 8 }}>
        Быстрая — пути без AI, около полуминуты. Полная — плюс живые сообщения боту по всем сценариям с оценкой ответов, идёт тремя частями подряд, шесть-восемь минут.
