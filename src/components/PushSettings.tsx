...
import {
  MORNING_TONES, MORNING_TOPICS, EVENING_THEMES,
  type MorningPrefs, type EveningPrefs, type MorningTone, type MorningTopic, type EveningTheme,
} from "@/lib/morningPrefs";

type L = {
  mTitle: string; mSub: string; mOn: string; toneLabel: string; topicsLabel: string;
  weekday: string; weekend: string; same: string; def: string; tzNote: (tz: string) => string;
  styleLabel: string; stylePh: string; off: string;
  eTitle: string; eSub: string; eOn: string; themesLabel: string; allThemes: string;
  customLabel: string; customPh: string; add: string; preview: string; loading: string; saved: string;
  tone: Record<MorningTone, string>; topic: Record<MorningTopic, string>; theme: Record<EveningTheme, string>;
};

const STR: Record<string, L> = {
  ru: {
    mTitle: "Утренний пуш", mSub: "Выбери тон, темы, время и стиль — ИИ соберёт сообщение под тебя.",
    mOn: "Отправлять утром", toneLabel: "Тон", topicsLabel: "Темы", weekday: "Время (будни)", weekend: "Время (выходные)",
    same: "Как в будни", def: "По умолчанию (~08:00)", tzNote: (tz) => `Твой пояс: ${tz}`,
    styleLabel: "Свой стиль (необязательно)", stylePh: "Напр.: коротко, без эмодзи, обращайся «капитан»",
    ...
    eOn: "Отправлять вечером", themesLabel: "Темы вопросов", allThemes: "Ничего не выбрано = все темы.",
  ...
  en: {
    mTitle: "Morning push", mSub: "Pick the tone, topics, time and style — the AI tailors your message.",
    mOn: "Send in the morning", toneLabel: "Tone", topicsLabel: "Topics", weekday: "Time (weekdays)", weekend: "Time (weekend)",
    same: "Same as weekdays", def: "Default (~08:00)", tzNote: (tz) => `Your zone: ${tz}`,
    styleLabel: "Your style (optional)", stylePh: "E.g.: short, no emoji, call me “captain”",
    ...
    eOn: "Send in the evening", themesLabel: "Question themes", allThemes: "Nothing selected = all themes.",
  ...
  uk: {
    mTitle: "Ранковий пуш", mSub: "Обери тон, теми, час і стиль — ІІ збере повідомлення під тебе.",
    mOn: "Надсилати вранці", toneLabel: "Тон", topicsLabel: "Теми", weekday: "Час (будні)", weekend: "Час (вихідні)",
    same: "Як у будні", def: "За замовчуванням (~08:00)", tzNote: (tz) => `Твій пояс: ${tz}`,
    styleLabel: "Свій стиль (необов'язково)", stylePh: "Напр.: коротко, без емодзі, звертайся «капітане»",
    ...
    eOn: "Надсилати ввечері", themesLabel: "Теми питань", allThemes: "Нічого не обрано = усі теми.",
  ...
  fr: {
    mTitle: "Push du matin", mSub: "Choisis le ton, les thèmes, l'heure et le style — l'IA s'adapte.",
    mOn: "Envoyer le matin", toneLabel: "Ton", topicsLabel: "Thèmes", weekday: "Heure (semaine)", weekend: "Heure (week-end)",
    same: "Comme en semaine", def: "Par défaut (~08:00)", tzNote: (tz) => `Ton fuseau : ${tz}`,
    styleLabel: "Ton style (facultatif)", stylePh: "Ex. : court, sans emoji, appelle-moi « capitaine »",
    ...
    eOn: "Envoyer le soir", themesLabel: "Thèmes des questions", allThemes: "Rien de sélectionné = tous les thèmes.",
...
export default function PushSettings({ locale, initial }: { locale: string; initial: MorningPrefs }) {
  ...
  const [draft, setDraft] = useState(""); // черновик новой подсказки
